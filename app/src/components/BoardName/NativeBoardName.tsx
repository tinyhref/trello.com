...
import { useNativeBoardNameFragment } from './NativeBoardNameFragment.generated';
import { useTrelloUpdateBoardNameMutation } from './TrelloUpdateBoardNameMutation.generated';

// eslint-disable-next-line @trello/less-matches-component
import * as styles from './BoardName.module.less';

/**
 * This instance of BoardName is currently experimental and uses native (server-side)
 * graphql data. This is the new version of the BoardName component that will eventually
 * replace the old version.  At that time, rename this component to `BoardName` and
 * remove `BoardEntry` (and revise this comment!).
 * @param boardId GraphQL node id for the board we're currently rendering
 */
export const NativeBoardName: FunctionComponent<BoardNameProps> = ({
  boardId,
}) => {
  ...
  const [updateBoardName] = useTrelloUpdateBoardNameMutation();

  const { data: board } = useNativeBoardNameFragment({
    from: { id: boardId },
  });
  ...
  const onEndEditing: FocusEventHandler<HTMLInputElement> = useCallback(
    async (e) => {
      ...
      try {
        const { data } = await updateBoardName({
          variables: {
            boardId,
            name: newBoardName,
          },
        ...
        if (data?.trello.updateBoardName?.success === false) {
          throw data.trello?.updateBoardName?.errors?.[0];
        }
