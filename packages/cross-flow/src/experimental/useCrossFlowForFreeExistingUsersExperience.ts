...
const EXPERIENCE_KEY = 'freeExistingUserExperience';

export const useCrossFlowForFreeExistingUsersExperience = ({
  source,
  workspaceId,
  boardId,
  hasValidAaSession,
  ...
  // 7. No paid workspace (standard, premium or enterprise)
  const { hasNoPaidWorkspaces } = useHasNoPaidWorkspaces();

  // 8. Check that *HomeScreen placements are rolled out for the user
